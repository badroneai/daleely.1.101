...
// Place-value tool — NEW, built on the Tool Playbook (Batch 5).
// Book skill: القيمة المنزلية. Levels قيمة الرقم / تكوين العدد / المقارنة; number
// size from the grade scope in CURRICULUM_MATRIX.
...
import {
  availableLevels,
  generateQuestion,
  type PlaceValueLevel,
  type PlaceValueQuestion,
  type PlaceValueScope,
} from "@/lib/tools/place-value/engine";
import type { GradeLevel } from "@/lib/types";
import TeachExample from "./TeachExample";

const SLUG = "place-value";

interface PlaceValueProps {
  grade: GradeLevel | "all";
}

export default function PlaceValue({ grade }: PlaceValueProps) {
  const scope = useMemo<PlaceValueScope>(() => {
    const s = getToolScope(SLUG, grade);
    return s && typeof s.digits === "number" ? s : { digits: 0 };
  }, [grade]);
  ...
  const [level, setLevel] = useState<PlaceValueLevel | null>(null);
  ...
  const [teach, setTeach] = useState<PlaceValueQuestion | null>(null);

  const speechAbort = useRef<AbortController | null>(null);
  useEffect(() => () => speechAbort.current?.abort(), []);

  const speak = (q: PlaceValueQuestion) => {
  ...
  const startLevel = (lvl: PlaceValueLevel) => {
  ...
  if (scope.digits === 0 || levels.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-xl text-gray-700">لا يوجد تدريب على القيمة المنزلية متاح لهذا الصف الدراسي.</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <p className="text-lg text-gray-700">اختر مهارة: قيمة الرقم، تكوين العدد، أو المقارنة.</p>
          {SpeechToggle}
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border-2 border-primary-500 rounded-xl p-6 text-center">
        ...
        <div className="flex gap-3 justify-center items-center">
          <label htmlFor="pv-answer" className="sr-only">إجابتك</label>
          <input
            id="pv-answer"
            type="text"
            inputMode="numeric"
            ...
            onKeyDown={onKeyDown}
            disabled={!!feedback}
            className="input-field text-center text-3xl w-40"
            placeholder="؟"
            autoFocus
            autoComplete="off"
          />
          ...
          {feedback && (
            <div className={feedback.correct ? "text-green-700" : "text-red-700"}>
              <p className="text-xl font-bold">
                {feedback.correct ? `✓ أحسنت! ${toArabicDigits(question!.answer)} صحيحة` : `✗ الإجابة الصحيحة: ${toArabicDigits(question!.answer)}`}
              </p>
