...
// Fractions tool — NEW, built on the Tool Playbook (Batch 6).
// Book skill: الكسور. Levels تمثيل الكسر / البسط والمقام / مقارنة الكسور.
// Denominators come from the grade scope in CURRICULUM_MATRIX.
...
import {
  ...
  type FractionsScope,
} from "@/lib/tools/fractions/engine";
...
function FractionBar({ num, den }: { num: number; den: number }) {
  ...
  const seg = w / den;
  return (
    <svg viewBox="0 0 240 60" className="w-64 h-16 mx-auto" role="img" aria-label="شكل مقسوم إلى أجزاء متكافئة">
...
export default function Fractions({ grade }: FractionsProps) {
  const scope = useMemo<FractionsScope>(() => {
  ...
  const speak = (q: FractionQuestion) => {
  ...
  if (levels.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-xl text-gray-700">تدريب الكسور متاح من الصف الثالث الابتدائي فما فوق.</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <p className="text-lg text-gray-700">اختر مهارة: تمثيل الكسر، البسط والمقام, أو مقارنة الكسور.</p>
          {SpeechToggle}
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border-2 border-primary-500 rounded-xl p-6 text-center">
        <p className="text-2xl font-bold text-gray-900 mb-4 leading-relaxed">{question?.prompt}</p>
        {question?.shape && <FractionBar num={question.shape.num} den={question.shape.den} />}

        {question?.answerKind === "input" ? (
          <div className="flex gap-3 justify-center items-center mt-4">
            <label htmlFor="fr-answer" className="sr-only">إجابتك</label>
            <input
              id="fr-answer"
              type="text"
              inputMode="numeric"
              ...
              onKeyDown={onKeyDown}
              disabled={!!feedback}
              className="input-field text-center text-3xl w-28"
              placeholder="؟"
              autoFocus
              autoComplete="off"
            />
