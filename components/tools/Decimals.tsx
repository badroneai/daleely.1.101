...
// Decimals tool — NEW (grade-5 gap #1), built on the Tool Playbook.
// Book skill: الكسور العشرية. Levels اقرأ / مقارنة / تحويل; number of decimal
// places from the grade scope in CURRICULUM_MATRIX.
...
import {
  ...
  type DecimalsScope,
} from "@/lib/tools/decimals/engine";
...
export default function Decimals({ grade }: DecimalsProps) {
  const scope = useMemo<DecimalsScope>(() => {
    const s = getToolScope(SLUG, grade);
    return s && typeof s.places === "number" ? s : { places: 0 };
  }, [grade]);
  ...
  const [question, setQuestion] = useState<DecimalQuestion | null>(null);
  ...
  if (scope.places === 0 || levels.length === 0) {
    return (
      <div className="text-center py-12">
        <p className="text-xl text-gray-700">تدريب الكسور العشرية متاح من الصف الرابع الابتدائي فما فوق.</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <p className="text-lg text-gray-700">اختر مهارة: قراءة الكسر العشري، المقارنة، أو التحويل.</p>
          {SpeechToggle}
        </div>
      ...
      <div className="bg-white border-2 border-primary-500 rounded-xl p-6 text-center">
        ...
        {question?.answerKind === "input" ? (
          <div className="flex gap-3 justify-center items-center">
            <label htmlFor="dec-answer" className="sr-only">إجابتك</label>
            <input
              id="dec-answer"
              type="text"
              inputMode="numeric"
